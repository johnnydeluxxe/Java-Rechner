...
public class BerechnungVolumen{
	...
	public static double berechneDreieckVolumen(double grundflaeche){
		...
		while(run){ //Solange run == true ist läuft die schleife endlos
			if(grundflaecheKlasse != Integer.MIN_VALUE){ // der Wert wurde Standardmäßig auf MIN.VALUE gesetzt, Sollte dieser Wert noch vorhanden sein 
				...
				run = false; // die variable run bekommt nun den wert false damit die schleife beendet wird. Das Programm gibt Volume zurück. 
			}else{
